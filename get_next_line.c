...
/*   Updated: 2021/07/03 12:49:23 by olabrecq         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main()
{
    ...
    line_count = 1;
    ret = 0;
        fd = open("file_tester", O_RDONLY);
        while ((ret = get_next_line(fd, &line)) > 0)
        {
            printf("[ Return: %d ] | A line has been read #%d => %s\n", ret, line_count, line);
            line_count++;
            free(line);
        }
        printf("[ Return: %d ] | Last line has been read #%d %s\n", ret, line_count++, line);
        printf("\n");
        if (ret == -1)
            printf("-----------\n An error happened\n");
        else if (ret == 0)
